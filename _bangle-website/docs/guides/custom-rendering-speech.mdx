...
:book: Reed the [Prosemirror.Schema](https://prosemirror.net/docs/guide/#schema) guide if you want to dig deeper on what each of the fields in the schema mean.
...
And in the list of your plugins we add a plugin which tells Bangle
what `dom.Node` to render, which internally will be used by `@banglejs/react`
to mount your Speech component.
